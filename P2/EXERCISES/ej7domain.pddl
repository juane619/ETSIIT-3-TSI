(define (domain ej7-domain)	  
  (:requirements :strips :equality :typing :fluents)
  
  ;jugador_give: jugador que da objetos a OTRO robot.
  ;jugador_get: jugador que recibe los objetos de OTRO robot para entregarlos a los personajes.
  (:types jugador jugador_give jugador_get - jugador personaje objeto zona orient tipo)
 ...
 ;Predicados
  ...
  ;coger objeto
  (:action takeObject
	     :parameters (?j - jugador_give ?obj - objeto ?z - zona)
	     :precondition 
	     (
	       and
			(empty_hand ?j )
  ...
  (:action giveObject
	  :parameters (?j - jugador_get ?obj - objeto ?z - zona ?p - personaje)
	  :precondition 
	  (
			and
			(not (empty_hand ?j ))
	  ...
	  (
		  ...
		  (increase (points_player ?j) (points ?obj ?p))
		  (increase (how_many ?p) 1)
	  )
  )
  
  ;entregar objeto a robot
  (:action giveObjectRobot
	  :parameters (?j_give - jugador_give ?j_get - jugador_get ?obj - objeto ?z - zona)
	  :precondition 
	  (
			and
			(not (empty_hand ?j_give ))
			(en_mano ?j_give ?obj)
			(at_jugador ?j_give ?z)
			(at_jugador ?j_get ?z)
	  )
	  ...
	  (
		  and
		  (not(en_mano ?j_give  ?obj))
		  (empty_hand ?j_give )
		  (en_mano ?j_get ?obj)
		  (not (empty_hand ?j_get))
	  )
  )
